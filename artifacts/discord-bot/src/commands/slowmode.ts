...
export async function execute(interaction: ChatInputCommandInteraction) {
  ...
  const embed = new EmbedBuilder().setColor(0x5865f2).setTitle("⏱️ Slowmode Updated")
    .addFields({ name: "Channel", value: targetChannel.toString(), inline: true }, { name: "Slowmode", value: seconds === 0 ? "Disabled" : `${seconds}s`, inline: true }, { name: "Set By", value: interaction.user.tag, inline: true }).setTimestamp();
