...
export async function execute(interaction: ChatInputCommandInteraction) {
  ...
  if (toDelete.length === 0) return interaction.editReply("No eligible messages found.");
  const deleted = await channel.bulkDelete(toDelete, true);
  const embed = new EmbedBuilder().setColor(0x5865f2).setTitle("🗑️ Messages Purged")
    .addFields({ name: "Deleted", value: `${deleted.size} message(s)`, inline: true }, { name: "Channel", value: channel.toString(), inline: true }, { name: "By", value: interaction.user.tag, inline: true }).setTimestamp();
