...
export async function execute(interaction: ChatInputCommandInteraction) {
  ...
  const embed = new EmbedBuilder().setColor(0x5865f2).setTitle(`👤 ${target.tag}`)
  ...
  if (member) {
    embed.addFields(
      ...
      { name: `Roles (${member.roles.cache.size - 1})`, value: member.roles.cache.filter((r) => r.id !== interaction.guild!.id).sort((a, b) => b.position - a.position).map((r) => r.toString()).slice(0, 10).join(", ") || "None" },
